...
describe('data-first', () => {
  ...
  it('prefix array', () => {
    const result = takeLastWhile([1] as [number, ...Array<boolean>], constant(true));

    expectTypeOf(result).toEqualTypeOf<Array<boolean | number>>();
  });

  it('suffix array', () => {
    const result = takeLastWhile([1] as [...Array<boolean>, number], constant(true));
  ...
  it('union of arrays', () => {
    const result = takeLastWhile([] as Array<boolean> | Array<string>, constant(true));
