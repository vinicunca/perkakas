 ...
 * bounds.
 */
export function quickSelect<T>(data: ReadonlyArray<T>,
  index: number,
  compareFn: CompareFunction<T>): T | undefined {
...
function quickSelectImplementation<T>(
  ...
  return index === pivotIndex
    ? data[index]! // Once a pivot is chosen it's location is final, so if it matches the index we found out item!
    : quickSelectImplementation(
      data,
