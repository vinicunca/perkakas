import type { NonEmptyArray, PredIndexed, PredIndexedOptional } from './_types';
 ...
 * @signature
 *  groupBy(array, fn)
 *  groupBy.strict(array, fn)
 * @example
 *  groupBy(['one', 'two', 'three'], x => x.length); // => {3: ['one', 'two'], 5: ['three']}
 *  groupBy.strict([{a: 'cat'}, {a: 'dog'}] as const, prop('a')); // => {cat: [{a: 'cat'}], dog: [{a: 'dog'}]} typed Partial<Record<'cat' | 'dog', NonEmptyArray<{a: 'cat' | 'dog'}>>>
 ...
 *  groupBy(fn)(array)
 * @example
 *  pipe(['one', 'two', 'three'], groupBy(x => x.length)); // => {3: ['one', 'two'], 5: ['three']}
 * @dataLast
 * @indexed
 * @category Array
 */
