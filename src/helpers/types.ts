...
/**
 * This type attempts to detect when a type is a single literal value
 * (e.g. "cat"), and not anything else (e.g. "cat" | "dog", string, etc...)
 */
type IsSingleLiteral<K> =
  IsLiteral<K> extends true ? (IsUnion<K> extends true ? false : true) : false;

/**
 * This is copied from type-fest because currently
 * it isn't exported.
 * TODO: Check whenever this type is exported.
 */
...
export type GuardType<T, Fallback = never> = T extends (
  // eslint-disable-next-line ts/no-explicit-any -- we don't care about arguments types here
  x: any,
  // eslint-disable-next-line ts/no-explicit-any -- we don't care about arguments types here
  ...rest: any
) => x is infer U
