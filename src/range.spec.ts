...
describe('non-integer inputs', () => {
  ...
  it('floating-point end', () => {
    expect(range(1, 5.5)).toStrictEqual([1, 2, 3, 4, 5]);
  });

  it('floating-point snap prevents extra element', () => {
    expect(range(0, { end: 3, step: 0.1 })).toHaveLength(30);
  });

  it('very small range near zero is not snapped away', () => {
...
describe('lodash spec', () => {
  ...
  it('should infer the sign of `step` when only `start` and `end` are given - incrementing', () => {
  ...
  it('should treat falsey `start` as `0` - zero', () => {
    // Trivially correct!
  ...
  it('should coerce arguments to finite numbers - NaN', () => {
