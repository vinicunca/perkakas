...
it('returns true for an own property whose value is undefined', () => {
  expect(hasProp({ a: undefined }, 'a')).toBe(true);
});

it('returns false for a missing optional property', () => {
...
it('returns false for properties only on a custom prototype', () => {
  expect(
    hasProp(
...
it('returns true for own properties on a null-prototype object', () => {
  expect(
    hasProp(
...
describe('arrays', () => {
  it('returns true for an array index that exists', () => {
    expect(hasProp([1, 2, 3], 1)).toBe(true);
  });

  it('returns false for an array index past the end', () => {
    expect(hasProp([1, 2, 3], 5)).toBe(false);
  });

  it('returns true for the array length own property', () => {
    expect(hasProp([1, 2, 3], 'length')).toBe(true);
  });

  it('returns false for inherited array members', () => {
