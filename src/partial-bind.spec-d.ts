...
describe('simple case (all required, no rest params)', () => {
  ...
  it('should not accept array typed partial', () => {
    // @ts-expect-error [ts2345] - don't know how many args are bound
    partialBind(fn, ...([] as Array<number>));
  });

  it('should not accept tuple typed partial with suffix', () => {
    // @ts-expect-error [ts2345] - wrong arg type
...
describe('optional params', () => {
  ...
  it('should correctly type 2 partial args', () => {
...
describe('simple rest param case', () => {
  ...
  it('should accept tuple typed partial arg', () => {
    expectTypeOf(partialBind(fn, ...([] as Array<string>))).toEqualTypeOf<
      (...parts: ReadonlyArray<string>) => string
    >();
  });

  it('should accept tuple typed partial arg with prefix', () => {
    expectTypeOf(
      partialBind(fn, ...(['hello'] as [string, ...Array<string>])),
    ).toEqualTypeOf<(...parts: ReadonlyArray<string>) => string>();
  });

  it('should accept tuple typed partial arg with prefix and suffix', () => {
    expectTypeOf(
      partialBind(fn, ...(['hello', 'world'] as [string, ...Array<string>, string])),
    ).toEqualTypeOf<(...parts: ReadonlyArray<string>) => string>();
  });

  it('should not accept tuple typed partial arg with incorrect prefix', () => {
    // @ts-expect-error [ts2345] - wrong arg type
    partialBind(fn, ...([1, 'hello'] as [number?, ...Array<string>]));
  });

  it('should not accept tuple typed partial arg with incorrect suffix', () => {
    // @ts-expect-error [ts2345] - wrong arg type
...
describe('known issues!', () => {
  it('does not support readonly rest params', () => {
