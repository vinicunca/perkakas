...
import type { ClampedIntegerSubtract } from './clamped-integer-subtract';
import type { IntRangeInclusive } from './int-range-inclusive';
...
export type ArrayAt<T extends IterableContainer, I extends keyof T>
  ...
  = IsNumericLiteral<I> extends true
    // The result of a union of indices is a union of the results for each one, so we distribute the union.
    ? I extends unknown
      // We memoize the prefix (made of the required and optional parts) so we can reuse it several times within this type.
      ? [
          ...TupleParts<T>['required'],
          ...TupleParts<T>['optional'],
        ] extends infer Prefix extends ReadonlyArray<unknown>
          // The first two parts of the tuple, the required and the optional prefixes, are simple and TypeScript does a good job of typing them based on indexing the tuple directly.
          ? HasIndex<Prefix, I> extends true
            ? T[I]
            // The index is larger than the prefix so the result has to consider the rest element item type.
            : | TupleParts<T>['item']
