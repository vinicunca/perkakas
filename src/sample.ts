...
type SampledGeneric<T extends IterableContainer>
  ...
  = T[number] extends never
    ...
    : T extends readonly [infer First, ...infer Rest]
      ...
      // Stop the recursion also when we have an array, or the rest element of the tuple
      : Array<T[number]>;
...
>
  ...
  = Iteration['length'] extends N
    ? []
    // If the tuple has a defined (non-rest) element, cut it and add it to the output tuple.
