...
describe('known limitations', () => {
  ...
  it('map: prototype methods narrow the else branch to never', () => {
  ...
  it('set: prototype methods narrow the else branch to never', () => {
...
describe('data-last', () => {
  it('narrows union members under filter', () => {
    const data = [] as Array<{ a?: number; b?: number }>;
  ...
  it('rejects keys that can\'t exist on the piped data', () => {
    // @ts-expect-error [ts2345] - "naem" is a typo
    pipe({} as { name: string }, hasProp('naem'));
  });

  it('narrows the array element when filtering a broad Record', () => {
    const data = [] as Array<Record<string, unknown>>;

    expectTypeOf(filter(data, hasProp('foo'))).toEqualTypeOf<
      Array<{ [x: string]: unknown; foo: unknown }>
    >();
  });
});
